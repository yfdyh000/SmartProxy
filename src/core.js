...
(function () {
	...
	var settingsOperation = {
		...
		findProxyServerByName: function (name) {
			let proxy = settings.find(item => { return item.name === name });
			if (proxy !== undefined) return proxy;

			for (let subscription of settings.proxyServerSubscriptions) {
				let sub = subscription.proxies.find(item => { return item.name === name });
				if (sub !== undefined) return proxy;
			}
	...
	var proxyRules = {
		...
		getRuleBySource: function (source) {
			...
			let rule = settings.proxyRules(rule => { return rule.source == source });
